...
- `options(scorecraft.fork_mem_fraction = 0.75)` caps the fork workers
  by the memory available on Linux (`Inf` to disable), since forked
  workers duplicate the parent heap once the garbage collector runs.
- Seeded steps (the split, the consensus classifiers, every bootstrap)
  no longer change the random-number stream of the session:
  `.Random.seed` is restored on exit.
